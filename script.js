...
if (!elements.clawMachine || !elements.box || !elements.collectionBox || !elements.collectionArrow) {
  console.error('Không thể tìm thấy các phần tử DOM cần thiết');
}
...
class Toy extends WorldObject {
  ...
  collectToy(toy) {
    ...
    setTimeout(() => {
      ...
      if (!document.querySelector('.selected') && elements.collectionArrow) {
...
const horiBtn = new Button({
  ...
  pressAction: () => {
    ...
    vertRail.interval = setInterval(() => {
      if (isMovingRight) {
        if (vertRail.x >= machineWidth - armJoint.w - machineBuffer.x) {
          isMovingRight = false;
        } else {
          vertRail.x += 10;
        }
      } else {
        if (vertRail.x <= machineBuffer.x) {
          isMovingRight = true;
        } else {
          vertRail.x -= 10;
        }
      ...
      vertRail.setStyles();
      if (vertRail.moveWith[1]) {
        vertRail.moveWith[1].x = vertRail.x;
        vertRail.moveWith[1].setStyles();
      }
    }, 50);
  },
  releaseAction: () => {
...
const vertBtn = new Button({
  ...
  pressAction: () => {
    ...
    let isMovingUp = true;
    armJoint.interval = setInterval(() => {
      if (isMovingUp) {
        if (armJoint.y <= machineBuffer.y) {
          isMovingUp = false;
        } else {
          armJoint.y -= 10;
        }
      } else {
        if (armJoint.y >= machineTopHeight - machineBuffer.y) {
          isMovingUp = true;
        } else {
          armJoint.y += 10;
        }
      ...
      armJoint.setStyles();
      if (armJoint.moveWith[0]) {
        armJoint.moveWith[0].y = armJoint.y;
        armJoint.moveWith[0].setStyles();
      }
    }, 50);
  },
  releaseAction: () => {
...
class Game {
  // Thêm biến để theo dõi hướng
  direction = {
    left: 1,    // 1 là sang trái, -1 là sang phải
    up: 1       // 1 là lên trên, -1 là xuống dưới
  };
  ...
  moveLeft() {
    if (this.x > 0) {
      ...
      // Đảo ngược hướng khi chạm biên trái
      this.direction.left = -1;
    }
    
    if (this.x >= this.canvas.width - this.width) {
      // Đảo ngược hướng khi chạm biên phải
      this.direction.left = 1;
    }
    
    // Di chuyển theo hướng hiện tại
    this.x += this.speed * (this.direction.left * -1);
  }
  ...
  moveUp() {
    if (this.y > 0) {
      ...
      // Đảo ngược hướng khi chạm biên trên
      this.direction.up = -1;
    }
    
    if (this.y >= this.canvas.height - this.height) {
      // Đảo ngược hướng khi chạm biên dưới
      this.direction.up = 1;
    }
    
    // Di chuyển theo hướng hiện tại
    this.y += this.speed * (this.direction.up * -1);
  }
